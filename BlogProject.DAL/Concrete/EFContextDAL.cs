...
namespace BlogProject.DAL.Concrete
{
    public static class EFContextDAL
    {
        public static IServiceCollection AddScopedDAL(this IServiceCollection services) 
        {
            ...
            {
                string blogConnection = @"Data source=MAHMUT_TAYLAN\SQLEXPRESS;Initial catalog=BLOGDB_MAHMUT;Integrated security=true; trustServerCertificate=true";
                options.UseSqlServer(blogConnection);
                options.UseLazyLoadingProxies();
            })
                .AddDbContext<AppIdentityContext>(options =>
                {
                    string identityConnection = @"Data source=MAHMUT_TAYLAN\SQLEXPRESS;Initial catalog=IDENTITYDB_MAHMUT;Integrated security=true; trustServerCertificate=true;";
