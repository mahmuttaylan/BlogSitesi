...
namespace BlogProject.BLL.Services.Concrete
{
    public class ArticleService : IArticleService
    {
        ...
        private readonly ClaimsPrincipal _user;     //Login kullanıyıcı bulmak için
       ...
       public ArticleService(IArticleRepository articleRepo, IMapper mapper, IUnitOfWork unitOfWork, IHttpContextAccessor httpContextAccessor, IImageHelper imageHelper, IImageRepository imageRepo)
        {
            ...
            _user = httpContextAccessor.HttpContext.User;       //Login kullanıyıcı bulmak için
        ...
        public async Task CreateArticle(ArticleCreateVM articleCreateVM)
        {
            ...
            await _unitOfWork.SaveAsync();      //yazmazsak image tabloya kayıt olmadığı için ıd=0 oluyor. aşağıda kullanırken image null geliyor. çünkü id otomatik olduğu için kayıt olana kadar 0 yazıyor.
