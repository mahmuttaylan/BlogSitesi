...
namespace BlogProject.BLL.Helpers.Images
{
    public class ImageHelper : IImageHelper
    {
        ...
        private const string imgFolder = "images";                          //Klasör isimleri
        ...
        //name = makalemizin başlığı olacak. resmi o isimle kayıt edeceğiz.
        public async Task<ImageUploadedVM> Upload(string name, IFormFile imageFile, ImageType imageType, string folderName = null)
        {
            ...
            string oldFileName = Path.GetFileNameWithoutExtension(imageFile.FileName);  //uzantısız dosya adı
            string fileExtension = Path.GetExtension(imageFile.FileName);               //uzantıyı getirir

            name = ReplaceInvalidChars(name);        //adı yazdığımız metoda göre sadeleştir   
            DateTime dateTime = DateTime.Now;
            string newFileName = $"{name}_{dateTime.Millisecond}{fileExtension}";   //aynı isimli resimler olmaması için Datetime'ın millisecond'ını kullandık. Dosya uzantısını ekledik. (resmi kaydederken makale adını kullanıyorduk. aynı makalede birden fazla resim olabilir.)
